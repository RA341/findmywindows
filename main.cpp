...
bool RegisterGlobalHotkey() {
    ...
    if (RegisterHotKey(nullptr, HOTKEY_ID, MOD_ALT, 0x31)) {
        // 0x31 is virtual key code for '1'
        std::cout << "Global hotkey Alt+1 registered successfully!" << std::endl;
        return true;
    }
...
void UnregisterGlobalHotkey() {
    ...
    std::cout << "Hotkey unregistered." << std::endl;
}

void handle_alt1();

void MessageLoop() {
    std::cout << "Listening for Alt+1" << std::endl;
...
void handle_alt1() {
    const auto windows = ListWindowsByDesktop(true);
    if (windows.empty()) {
        std::cout << "No windows found " << windows.size() << std::endl;
        return;
    }
    BringWindowToFront(windows[1].hwnd);

    std::cout << "Alt+1 pressed! Hello from global hotkey!" << std::endl;
}
